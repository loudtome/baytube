...
def get_hourly(url, ua, timeout, step=1, count=24):
    """Air temp (°F) + rain chance (%) from the NWS hourly forecast (a land
    gridpoint), sampled every `step` hours for `count` points (24h for the
    temp/rain graph). Temperature already comes in °F."""
